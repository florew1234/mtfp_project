...
export class ListRequeteAdjointComponent implements OnInit {
  ...
  transmettreReponse() {
    ...
    if (this.selected_data.reponse.length > 0) {
      let check = this.selected_data.reponse.filter((item) => (item.typeStructure=='SRU Secondaire'));
      if(check.length == 0){
    ...
    if (this.selected_data.reponse.length > 0) {
      let check = this.selected_data.reponse.filter((item) => (item.typeStructure=='SRU Secondaire' && item.siTransmis==1));
      if(check.length != 0){

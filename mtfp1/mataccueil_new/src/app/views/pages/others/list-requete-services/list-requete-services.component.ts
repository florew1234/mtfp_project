...
export class ListRequeteServicesComponent implements OnInit {
  ...
  relancerPreocuppationType(){
    ...
    if (this.selected_data.parcours.length > 0) {
      this.selected_data.parcours.forEach(item => {
        this.cpt++;
        if (this.cpt == this.selected_data.parcours.length && item.idStructure == this.user.agent_user.idStructure){
            AlertNotif.finish("Erreur", "Impossible de faire une relance car le responsable structure a déjà donné sa réponse", 'error');
            return;
          }
      })
    }
  ...
  saveAffectation(value) {
    ...
    // }
    if (this.selected_data.reponse.length > 0) {
      AlertNotif.finish("Erreur", "Vous ne pouvez plus répondre cette requete car une réponse a été déjà proposée.", 'error');
      return;
    }
  ...
  transmettreReponse() {
    ...
    if (this.selected_data.reponse.length > 0) {
      let check = this.selected_data.reponse.filter((item) => (item.typeStructure=='Service'));
      if(check.length == 0){
    ...
    if (this.selected_data.reponse.length > 0) {
      let check = this.selected_data.reponse.filter((item) => (item.typeStructure=='Service' && item.siTransmis==1));
      if(check.length != 0){

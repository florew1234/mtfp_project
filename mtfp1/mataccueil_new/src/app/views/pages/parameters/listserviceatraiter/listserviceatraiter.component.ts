...
export class ListserviceatraiterComponent implements OnInit {
  ...
  create(value) {
    var getNbreJours=0;
    if(value.delaiFixe==true){getNbreJours=value.nbreJours;}
    value.nbreJours=getNbreJours
    var geturl=null
    if(value.access_online==true){geturl=value.access_url;}
    value.access_url=geturl,
    value.cout=value.cout=="" ? 0 : +value.cout  
    value.cout=value.cout=="" ? 0 : +value.cout  
    value.idEntite=this.user.idEntite
    
    this.prestationService.create(value).subscribe((res: any) => {

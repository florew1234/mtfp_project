...
import { RdvParamService } from '../../../../core/_services/rdv-param.service';
...
export class ListerdvparametreComponent implements OnInit {
  ...
  init(){
    this._temp=[]
    this.data=[]
    this.rdvParamService.getAll(this.user.idEntite).subscribe((res:any)=>{
      this.spinner.hide();
      this.data=res
  ...
  create(value){
    ...
    this.rdvParamService.create(value).subscribe((res:any)=>{
  ...
  edit(value) {
    ...
    value.idEntite=this.user.idEntite
    this.rdvParamService.update(value,this.selected_data.id).subscribe((res)=>{
      this.modalService.dismissAll()
      this.init()

...
<head>
    ...
    <title>TAUX DE DIGITALISATION</title>
			...
			<div>
				<div>
                    <table class="tb" style="border : 1px solid black; width : 100%;">
						<thead>
                            <tr>
                                <th class="tb">Structure</th>
                                <th class="tb cent">Totale prestation</th>
                                <th class="tb cent">Sollicité</th>
                                <th class="tb cent">Taux de digitalisation</th>
							</tr>
						</thead>
                        @if(count($data) != 0)
						<tbody>
                            @foreach($data as $dt)
								<tr>
									<td class="ts" style="padding:8px" >{{$dt['libelle']}}</td>
                                    <td class="tb" >{{$dt['total']}}</td>
									<td class="tb" >{{$dt['totalOnline']}}</td>
									<td class="tb" >{{$dt['taux']}}</td>
								</tr>
							@endforeach
						</tbody>
                        @else
                            <tr><th colspan="4"><h3 class="text-center"><strong>Aucune information trouvée</strong></h3></th></tr>                             
                        @endif   
					</table>                    
				</div>
			</div>

...
class ServiceController extends Controller {
	...
	static public  function tab_Data_Rapport(Request $req){
		...
		// 						->distinct()
		// 						->get();
		$structures = Requete::join('outilcollecte_service','outilcollecte_service.id','outilcollecte_requete.idPrestation')
							 ->join('outilcollecte_structure','outilcollecte_structure.id','outilcollecte_service.idParent')
							 ->whereBetween('dateRequete',[$dedate,$fidate])
							 ->where('plainte',2)
							 ->whereHas('affectation.structure',function($q)use($idEntite){
								$q->whereIn('type_s',['dt','dc'])->where('idEntite',$idEntite)->where('active',1);
							})
							->select(DB::raw("
							outilcollecte_structure.id,
							outilcollecte_structure.sigle,
							count(*) total,
							SUM(CASE WHEN traiteOuiNon = 1 then 1 else 0 end) totalTraite,
							SUM(CASE WHEN traiteOuiNon = 0 then 1 else 0 end) totalNTraite,
							SUM(CASE WHEN archiver = 1 then 1 else 0 end) totalArcive,
							SUM(CASE WHEN horsDelai = 3 and traiteOuiNon = 1 then 1 else 0 end) totalTraiteDansLesDelais,
							SUM(case WHEN outilcollecte_service.nbreJours = 0 
									then 0 
									else DATEDIFF(outilcollecte_requete.dateReponse,outilcollecte_requete.created_at) 
									end) totalTempTraitement
							"))
							->groupBy('outilcollecte_structure.sigle')
							->orderBy('outilcollecte_structure.sigle','asc')
							->get();
